...
let _ = 
  ...
  bitmatch bits with
    | { id: 16;
      ...
      ignore(payload);
      Printf.printf "DNS:\n";
      Printf.printf "  id:%08x\n" id;
      Printf.printf "  qr:%B\n" qr;
      Printf.printf "  opcode:%02x\n" opcode;
      Printf.printf "  aa:%B tc:%B rd:%B ra:%B\n" aa tc rd ra;
      Printf.printf "  z:%02x\n" z;
      Printf.printf "  rcode:%02x\n" rcode;
      Printf.printf "  qdcount:%d ancount:%d nscount:%d arcount:%d\n"
        qdcount ancount nscount arcount

    | { _ } -> Printf.eprintf "invalid data: %s\n" fn
